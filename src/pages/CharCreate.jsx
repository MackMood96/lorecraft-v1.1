...
export default function CharCreate() {
  ...
  function handleSlider(attrId, newVal) {
    ...
    // Dragging left — always allow, frees up points
    if (delta < 0) {
      setBaseAttrs(prev => ({ ...prev, [attrId]: clamped }))
      return
    }

    // Dragging right — only allow if enough points remain
    if (delta > 0 && pointsLeft >= delta) {
      setBaseAttrs(prev => ({ ...prev, [attrId]: clamped }))
    }
  }
  ...
  async function handleBegin() {
    ...
    try {
      ...
      const { data: player, error: playerError } = await supabase
        .from('players')
        .insert({
          name: name.trim(), class: cls.name, race: race.name, affinity: affinity.name,
          appearance: appearance.trim(), backstory: backstory.trim(),
          hp: computedHp, max_hp: computedHp, gold: 10, level: 1,
          inventory: startingInventory, attributes: finalAttrs,
          equipment: cls.equipment, abilities: [],
      ...
      await supabase.from('players').update({ avatar_url: avatarPollUrl, avatar_seed: seed, avatar_prompt: avatarPrompt }).eq('id', player.id)
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: 'var(--bg)', overflowY: 'auto' }}>
        ...
        <div style={{ padding: '16px 20px 0', flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
            ...
            <div style={{
              ...
              color: pointsLeft === 0 ? '#27ae60' : pointsLeft < 15 ? '#f39c12' : 'var(--gold)',
              padding: '3px 10px', borderRadius: '4px',
              background: pointsLeft === 0 ? 'rgba(39,174,96,0.1)' : 'rgba(201,168,76,0.1)',
              border: `1px solid ${pointsLeft === 0 ? '#27ae60' : 'rgba(201,168,76,0.3)'}`
            }}>
                ...
                <div key={attr.id} style={{ background: 'var(--bg2)', border: `1px solid ${attr.color}33`, borderLeft: `3px solid ${attr.color}`, borderRadius: '8px', padding: '10px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      ...
                      {bonus !== 0 && (
                        <span style={{ fontSize: '10px', color: bonus > 0 ? '#27ae60' : '#c0392b', marginLeft: '4px' }}>
                          {bonus > 0 ? `+${bonus}` : bonus}
                        </span>
                      )}
                    </div>
                  ...
                  <input
                    ...
                    className={`attr-slider slider-${attr.id}`}
                    style={{
                      width: '100%',
                      height: '6px',
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '4px' }}>
                    ...
                    {cls.attrCaps?.[attr.id] && (
                      <span style={{ fontSize: '8px', color: '#c0392b88' }}>cap: {cap}</span>
                    )}
      ...
      <style>{`
        input[type=range].attr-slider {
          -webkit-appearance: none;
          appearance: none;
        }
        input[type=range].attr-slider::-webkit-slider-runnable-track {
          height: 6px;
          border-radius: 3px;
        }
        input[type=range].attr-slider::-webkit-slider-thumb {
          -webkit-appearance: none;
          appearance: none;
          width: 20px;
          height: 20px;
        ...
        input[type=range].attr-slider::-moz-range-thumb {
          width: 20px;
          height: 20px;
          border-radius: 50%;
          background: var(--gold);
        ...
        input[type=range].attr-slider::-moz-range-track {
          height: 6px;
          border-radius: 3px;
          background: transparent;
        }
        input[type=range].attr-slider:focus {
          outline: none;
        }
      `}</style>
    </div>
